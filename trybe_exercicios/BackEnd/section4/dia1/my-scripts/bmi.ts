...
const verificationCategory = (valor: number):string => {
    if (valor < 18.5) {
...
const bmi = () => {
  ...
  const weight:number = redline.questionFloat('Qual o seu peso? \n');
  const height:number = redline.questionFloat('Qual a sua altura? \n');
